...
export class LmnChevronDownIcon extends LmnIconBase {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (this.animate()) {
        this.player = this.engine.play(el, { y: [0, 3, 0] }, { config: { duration: 400, easing: 'ease-in-out', delay: 0, disabled: false } });
      } else {
        this.player = this.engine.play(el, { y: [0] }, { config: { duration: 200, easing: 'ease-out', delay: 0, disabled: false } });
      }
    });
  }
}
