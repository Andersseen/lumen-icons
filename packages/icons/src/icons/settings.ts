...
export class LmnSettingsIcon extends LmnIconBase {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (this.animate()) {
        this.player = this.engine.play(el, { rotate: [0, 90] }, { config: { duration: 500, easing: 'cubic-bezier(0.34, 1.56, 0.64, 1)', delay: 0, disabled: false } });
      } else {
        this.player = this.engine.play(el, { rotate: [90, 0] }, { config: { duration: 300, easing: 'ease-out', delay: 0, disabled: false } });
      }
    });
  }
}
