    ...
    <section class="py-20" aria-labelledby="features-heading">
      <div class="mx-auto max-w-6xl px-4">
        <div class="mb-12 text-center">
          <h2 id="features-heading" class="text-3xl font-bold tracking-tight text-foreground">
            Designed for Angular
          </h2>
          <p class="mt-3 text-secondary-foreground">Every decision optimised for the Angular ecosystem.</p>
        </div>

        <ul class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3" role="list">
          @for (f of features; track f.title) {
            <li class="rounded-2xl border border-border bg-background p-6 shadow-sm dark:border-border dark:bg-card">
              <div class="mb-4 flex h-10 w-10 items-center justify-center rounded-xl border border-border bg-muted dark:border-border dark:bg-secondary"
                [class]="f.iconColor">
                @switch (f.icon) {
              ...
              <p class="mt-1.5 text-sm leading-relaxed text-secondary-foreground">{{ f.description }}</p>
            </li>
          }
        </ul>
      </div>
