...
- Sources: vendored Heroicons v2.2.0 in `packages/icons/svg/` (outline + solid variants, MIT), plus ~38 custom icons preserved by the generator's extraction pipeline. No `heroicons` npm dependency.
...
- **Tooling:** Angular 21 (zoneless — no zone.js anywhere) · AnalogJS 2.4 (SSR off, static SPA) · Vite 8 · Vitest 4 + Testing Library (zoneless TestBed) · Playwright · Tailwind v4 (app only) · @voltui/components (app chrome) · pnpm 10 · husky + commitlint (conventional commits).
...
- **2026-07-27** — Zero third-party icon deps + zoneless. Heroicons SVGs (v2.2.0, MIT, 324 outline + 324 solid) vendored into `packages/icons/svg/` — the `heroicons` npm dependency is removed and the generator + `rebuild-legacy-icons.mjs` read the vendored copies; regeneration verified **byte-identical** (zero icon diff). App and TestBed are now explicitly zoneless (`provideZonelessChangeDetection` in `app.config.ts`, `setupTestBed({ zoneless: true })`); `zone.js` dependency removed. Zoneless exposed a latent a11y bug in `icons-sidebar.html` (`<label for>` pointing at a non-labellable `<volt-input>`) — fixed with `aria-label`, matching the mobile-controls pattern. `pnpm run check` green.
